...
class AgentOrchestrator:
    ...
    async def process_lecture(
        self, lecture_id: str, content: str, title: str
    ) -> Dict[str, Any]:
        """
        ...
        analysis = await self.comprehension_agent.execute(content=content, title=title)

        # Step 2: Store lecture content as embeddings
        embedding_ids = await self.memory_manager.store_lecture_memory(
            lecture_id=lecture_id, content=content
        )
