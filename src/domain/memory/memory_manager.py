...
class MemoryManager:
    ...
    async def store_lecture_memory(self, lecture_id: str, content: str) -> List[str]:
        """
        ...
        embedding_ids = await self.embedding_repo.create_batch(
            lecture_id=lecture_id, chunks=chunks, embeddings=embeddings
        )

        return embedding_ids

    async def retrieve_relevant_context(
        self, query: str, lecture_ids: List[str], top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        results = await self.embedding_repo.search_similar(
            query_embedding=query_embedding, lecture_ids=lecture_ids, limit=top_k
        )
