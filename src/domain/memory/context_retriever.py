...
class ContextRetriever:
    ...
    async def retrieve_for_test_generation(
        self, lecture_ids: List[str], topic: str, top_k: int = 5
    ) -> str:
        """
        ...
        context_chunks = await self.memory_manager.retrieve_relevant_context(
            query=topic, lecture_ids=lecture_ids, top_k=top_k
        )
    ...
    async def retrieve_for_evaluation(
        self, lecture_ids: List[str], question: str, student_answer: str, top_k: int = 3
    ) -> str:
        """
        ...
        context_chunks = await self.memory_manager.retrieve_relevant_context(
            query=query, lecture_ids=lecture_ids, top_k=top_k
        )
