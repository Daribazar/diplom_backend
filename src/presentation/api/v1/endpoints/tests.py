...
async def get_test(
    ...
    try:
        ...
        await _require_lecture_access(db, lecture_repo, course_repo, test.lecture_id, current_user)
        return _to_test_response(test)
    except (NotFoundError, UnauthorizedError, ValueError, PermissionError) as e:
        raise map_common_domain_error(e)
...
async def get_lecture_tests(
    ...
    try:
        await _require_lecture_access(db, lecture_repo, course_repo, lecture_id, current_user)
        tests = await test_repo.get_by_lecture(lecture_id)
        if current_user.role == ROLE_STUDENT:
            tests = [t for t in tests if t.created_by == current_user.id]
        return TestListResponse(total=len(tests), tests=[_to_test_response(t) for t in tests])
    except (NotFoundError, UnauthorizedError, ValueError, PermissionError) as e:
        raise map_common_domain_error(e)
