...
PERFECT_SCORE_FEEDBACK = (
    "Маш сайн! Та энэ тестийг 100% зөв хийлээ. " "Энэ эрчээрээ үргэлжлүүлээрэй."
)
...
def _create_evaluation_agent() -> EvaluationAgent:
    llm = LLMFactory.create_default_adapter()
    return EvaluationAgent(llm)


def _evaluation_repositories(
    db: AsyncSession,
) -> tuple[StudentAttemptRepository, TestRepository]:
    return StudentAttemptRepository(db), TestRepository(db)


def _build_submit_use_case(
    db: AsyncSession,
) -> tuple[SubmitTestUseCase, EvaluationAgent]:
    attempt_repo, test_repo = _evaluation_repositories(db)
    evaluation_agent = _create_evaluation_agent()
...
async def _resolve_overall_feedback(
    evaluation_agent: EvaluationAgent,
    attempt,
) -> str:
    if (attempt.percentage or 0) >= PERFECT_SCORE_THRESHOLD:
        return PERFECT_SCORE_FEEDBACK
    try:
        return await evaluation_agent._generate_overall_feedback(
            percentage=attempt.percentage,
            weak_topics=attempt.weak_topics or [],
            analytics=attempt.analytics or {},
        )
    except Exception:
        return OVERALL_FEEDBACK_CACHED
...
async def get_attempt_result(
    attempt_id: str, current_user: CurrentUser, db: AsyncSession = Depends(get_db)
):
    ...
    try:
        attempt = await _require_owned_attempt(
            attempt_repo, attempt_id, current_user.id
        )
        test = await test_repo.get_by_id(attempt.test_id)
        enriched_answers = _enrich_attempt_answers(attempt.answers, test)
        overall_feedback = await _resolve_overall_feedback(evaluation_agent, attempt)
        return EvaluationResponse(
            attempt_id=attempt.id,
            test_id=attempt.test_id,
            total_score=attempt.total_score,
...
async def get_test_attempts(
    test_id: str, current_user: CurrentUser, db: AsyncSession = Depends(get_db)
):
