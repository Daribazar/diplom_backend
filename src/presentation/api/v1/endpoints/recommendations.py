...
def _empty_recommendation(
    course_id: str, based_on_week: int, ai_advice: str
) -> CourseRecommendationResponse:
...
def _latest_week(lectures: list[LectureModel]) -> int:
    return lectures[-1].week_number if lectures else 0


async def _require_accessible_course(
    db: AsyncSession,
    course_id: str,
    ...
    can_access = await has_course_access(
        db, course_repo, course_id, current_user.id, current_user.role
    )
    if not can_access:
        raise HTTPException(
...
def _build_focus_areas(attempts: list[StudentAttemptModel]) -> list[FocusArea]:
    ...
    for topic, stats in weak_topics.items():
        ...
        priority = (
            ...
            else "medium" if weakness_ratio >= 0.4 else "low"
        )
        focus_areas.append(
            FocusArea(topic=topic, percentage=round(percentage), priority=priority)
        )
...
def _build_study_plan(focus_areas: list[FocusArea]) -> list[StudyPlanDay]:
    ...
    day1_tasks = [
        StudyTask(duration=30, description=f"{area.topic} сэдвийн материал дахин унших")
        for area in focus_areas[:3]
    ]
    day2_tasks = [
        StudyTask(duration=25, description=f"{area.topic}-тай холбоотой дасгал бодох")
        for area in focus_areas[:3]
    ]
...
async def get_course_recommendations(
    ...
    await _require_accessible_course(db, course_id, current_user)
    ...
    if not tests:
        return _empty_recommendation(
            course_id=course_id,
            based_on_week=_latest_week(lectures),
