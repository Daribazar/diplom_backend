...
async def process_lecture_task(self, lecture_id: str):
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            logger.info(
                f"Final status: {db_lecture_updated.status if db_lecture_updated else 'unknown'}"
            )
            logger.info(
                f"Key concepts count: {len(db_lecture_updated.key_concepts) if db_lecture_updated else 0}"
            )
