...
class UserModel(Base, TimestampMixin):
    ...
    id = Column(
        String, primary_key=True, default=lambda: f"user_{uuid.uuid4().hex[:12]}"
    )
    ...
    courses = relationship(
        "CourseModel", back_populates="owner", cascade="all, delete-orphan"
    )
    attempts = relationship(
        "StudentAttemptModel", back_populates="student", cascade="all, delete-orphan"
    )
