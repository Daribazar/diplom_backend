...
async def test_test_generation():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if (
            ...
            or not courses_response.json()["courses"]
        ):
        ...
        if not completed_lecture:
            print(
                "❌ No completed lectures found. Please wait for processing to complete"
            )
        ...
        for i, q in enumerate(test["questions"], 1):
            ...
            print(
                f"Difficulty: {q['difficulty']} | Bloom: {q.get('bloom_level', 'N/A')}"
            )
