...
async def test_complete_workflow():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        files = {
            "file": ("lecture1.pdf", pdf_buffer, "application/pdf")
        }
        data = {
            "course_id": course_id,
            "week_number": "1",
        ...
        for attempt in range(max_attempts):
            ...
            response = await client.get(
                f"{API_BASE}/lectures/{lecture_id}/status",
                headers=headers
            )
        ...
        response = await client.get(
            f"{API_BASE}/evaluations/attempt/{attempt_id}",
            headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            print(f"   Score: {attempt_data.get('score')}/{attempt_data.get('max_score')}")
