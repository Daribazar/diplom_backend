...
class EndpointTester:
    ...
    def add_result(self, category: str, endpoint: str, method: str, status: str, message: str = ""):
        """Add test result"""
        self.results.append({
            "category": category,
            "endpoint": endpoint,
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.add_result("System", "/", "GET", "❌ FAIL", f"Status: {response.status_code}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.add_result("System", "/health", "GET", "✅ PASS", f"DB: {data.get('database')}, Redis: {data.get('redis')}")
                else:
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    self.add_result("Auth", "/auth/register", "POST", "✅ PASS", f"User ID: {self.user_id}")
                elif response.status_code == 409:
                    console.print("⚠️  Register: User already exists (OK)")
                    self.add_result("Auth", "/auth/register", "POST", "⚠️  SKIP", "User exists")
                else:
                    console.print(f"❌ Register: Failed ({response.status_code})")
                    self.add_result("Auth", "/auth/register", "POST", "❌ FAIL", f"Status: {response.status_code}")
            except Exception as e:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.add_result("Auth", "/auth/login", "POST", "✅ PASS", "Token received")
                else:
                    console.print(f"❌ Login: Failed ({response.status_code})")
                    self.add_result("Auth", "/auth/login", "POST", "❌ FAIL", f"Status: {response.status_code}")
                    return False
            except Exception as e:
            ...
            if self.token:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.add_result("Auth", "/auth/me", "GET", "✅ PASS", f"Email: {data.get('email')}")
                    else:
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    self.add_result("Courses", "/courses", "POST", "✅ PASS", f"Course ID: {self.course_id}")
                else:
                    console.print(f"❌ Create course: Failed ({response.status_code})")
                    self.add_result("Courses", "/courses", "POST", "❌ FAIL", f"Status: {response.status_code}")
                    return False
            except Exception as e:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.add_result("Courses", "/courses", "GET", "✅ PASS", f"Total: {data.get('total')}")
                else:
            ...
            if self.course_id:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.add_result("Courses", f"/courses/{'{id}'}", "GET", "✅ PASS")
                    else:
                        console.print(f"❌ Get course: Failed")
                        self.add_result("Courses", f"/courses/{'{id}'}", "GET", "❌ FAIL")
                except Exception as e:
                    console.print(f"❌ Get course: Error - {e}")
                    self.add_result("Courses", f"/courses/{'{id}'}", "GET", "❌ ERROR", str(e))
            ...
            if self.course_id:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.add_result("Courses", f"/courses/{'{id}'}", "PATCH", "✅ PASS")
                    else:
                        console.print(f"❌ Update course: Failed")
                        self.add_result("Courses", f"/courses/{'{id}'}", "PATCH", "❌ FAIL")
                except Exception as e:
                    console.print(f"❌ Update course: Error - {e}")
                    self.add_result("Courses", f"/courses/{'{id}'}", "PATCH", "❌ ERROR", str(e))
    ...
    async def test_lectures(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                response = await client.get(
                    f"{API_BASE}/lectures/course/{self.course_id}",
                    headers=headers
                )
                if response.status_code == 200:
                    data = response.json()
                    console.print(f"✅ Get lectures: {data.get('total')} lectures")
                    self.add_result("Lectures", f"/lectures/course/{'{id}'}", "GET", "✅ PASS", f"Total: {data.get('total')}")
                else:
                    console.print(f"❌ Get lectures: Failed")
                    self.add_result("Lectures", f"/lectures/course/{'{id}'}", "GET", "❌ FAIL")
            except Exception as e:
                console.print(f"❌ Get lectures: Error - {e}")
                self.add_result("Lectures", f"/lectures/course/{'{id}'}", "GET", "❌ ERROR", str(e))
            
            # Note: File upload test requires actual PDF file
            console.print("⚠️  Lecture upload requires PDF file (skipping)")
            self.add_result("Lectures", "/lectures/upload", "POST", "⚠️  SKIP", "Requires PDF file")
    ...
    async def test_tests(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            self.add_result("Tests", "/tests/generate", "POST", "⚠️  SKIP", "Requires lecture")
            self.add_result("Tests", f"/tests/{'{id}'}", "GET", "⚠️  SKIP", "Requires test")
            self.add_result("Tests", f"/tests/lecture/{'{id}'}", "GET", "⚠️  SKIP", "Requires lecture")
    ...
    async def test_evaluations(self):
        ...
        self.add_result("Evaluations", f"/evaluations/submit/{'{id}'}", "POST", "⚠️  SKIP", "Requires test")
        self.add_result("Evaluations", f"/evaluations/attempt/{'{id}'}", "GET", "⚠️  SKIP", "Requires attempt")
        self.add_result("Evaluations", f"/evaluations/test/{'{id}'}/attempts", "GET", "⚠️  SKIP", "Requires test")
    ...
    def print_summary(self):
        ...
        for result in self.results:
            print(f"{result['category']:<15} {result['endpoint']:<35} {result['method']:<8} {result['status']:<12} {result['message']:<30}")
