...
async def test_complete_flow():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        print(f"   Key concepts: {', '.join(completed_lecture.get('key_concepts', [])[:3])}...")
        ...
        if 'by_difficulty' in analytics:
            ...
            for diff, stats in analytics['by_difficulty'].items():
                ...
                print(f"     - {diff.capitalize()}: {stats['correct']}/{stats['total']} ({acc:.0f}%)")
        
        if 'by_type' in analytics:
            print(f"\n   By Type:")
            for qtype, stats in analytics['by_type'].items():
                acc = stats['correct'] / stats['total'] * 100 if stats['total'] > 0 else 0
                print(f"     - {qtype.upper()}: {stats['correct']}/{stats['total']} ({acc:.0f}%)")
        ...
        print(f"Retrieve later: GET /api/v1/evaluations/attempt/{evaluation['attempt_id']}")
